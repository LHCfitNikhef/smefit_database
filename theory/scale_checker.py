...
for file in filesToCheck:
    ...
    with open(file, "r") as fi:
        ...
        if any(listmatch):
            matchedObject = next(
                (item for item in listmatch if isinstance(item, re.Match)), None
            )
        ...
        if any(listmatch):
            ...
            if process not in list(scaleset.keys()):
                continue
            #           if not(process in scaleset.keys): continue
            print(
                "ATLAS/CMS inclusive file = {} , process id = {}".format(file, process)
