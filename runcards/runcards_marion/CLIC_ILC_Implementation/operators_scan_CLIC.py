...
with open("scan_ops_eeWW_CLIC3000.mg5".format(job_id), 'w') as script:
    ...
    for op in op_list:
        if len(op) == 2:
            if os.path.isdir("./Events/%s" % "_".join(op)):
                continue
            script.write("launch eeWW_NP4_CLIC3000 --name=%s_CLICv2_1500GeV_pos80_sq\n" % "_".join(op))
            script.write("analysis=off\n")
            script.write("set polbeam2 0.8\n")
            script.write("set polbeam1 0.0\n") 
            write_op(op, script)
            ...
            if os.path.isdir("./Events/%s" % op):
                continue
            script.write("launch eeWW_NP2_CLIC3000 --name=%s_CLICv2_1500GeV_pos80_inter\n" % op)
            script.write("analysis=off\n")
            script.write("set polbeam2 0.8\n")
            script.write("set polbeam1 0.0\n") 
            write_op(op, script)
    ...
    for op in op_list:
        if len(op) == 2:
            if os.path.isdir("./Events/%s" % "_".join(op)):
                continue
            script.write("launch eeWW_NP4_CLIC3000 --name=%s_CLICv2_1500GeV_neg80_sq\n" % "_".join(op))
            script.write("analysis=off\n")
            script.write("set polbeam2 -0.8\n")
            script.write("set polbeam1 0.0\n") 
            write_op(op, script)
            ...
            if os.path.isdir("./Events/%s" % op):
                continue
            script.write("launch eeWW_NP2_CLIC3000 --name=%s_CLICv2_1500GeV_neg80_inter\n" % op)
            script.write("analysis=off\n")
            script.write("set polbeam2 -0.8\n")
            script.write("set polbeam1 0.0\n") 
            write_op(op, script)
