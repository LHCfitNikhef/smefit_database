    ...
    for op in op_list:
        if len(op) == 2:
            if os.path.isdir("./Events/%s" % "_".join(op)):
                continue
            script.write(
                "launch /home/marion/eeWW_NP4 --name=%s_ILC_500GeV_pos80_neg30_sq\n"
                % "_".join(op)
            )
            script.write("analysis=off\n")
            script.write("set polbeam2 0.8\n")
            ...
            if os.path.isdir("./Events/%s" % op):
                continue
            script.write(
                "launch /home/marion/eeWW_NP2 --name=%s_ILC_500GeV_pos80_neg30_inter\n"
                % op
            )
            script.write("analysis=off\n")
            script.write("set polbeam2 0.8\n")
    ...
    for op in op_list:
        if len(op) == 2:
            if os.path.isdir("./Events/%s" % "_".join(op)):
                continue
            script.write(
                "launch /home/marion/eeWW_NP4 --name=%s_ILC_500GeV_neg80_pos30_sq\n"
                % "_".join(op)
            )
            script.write("analysis=off\n")
            script.write("set polbeam2 -0.8\n")
            ...
            if os.path.isdir("./Events/%s" % op):
                continue
            script.write(
                "launch /home/marion/eeWW_NP2 --name=%s_ILC_500GeV_neg80_pos30_inter\n"
                % op
            )
            script.write("analysis=off\n")
            script.write("set polbeam2 -0.8\n")
